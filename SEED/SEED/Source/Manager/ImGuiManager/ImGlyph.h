...
namespace SEED{
    ...
    struct GlyphRanges{
        ...
        static inline std::vector<int> CreateJpGlyphSet(){
            ...
            // 全角記号・句読点など（U+3000 ～ U+303F）
            for(int cp = 0x3000; cp <= 0x303F; ++cp){
                glyphs.push_back(cp);
            }

            // ひらがな（U+3040 ～ U+309F）
            for(int cp = 0x3040; cp <= 0x309F; ++cp){
                glyphs.push_back(cp);
            }

            // カタカナ（U+30A0 ～ U+30FF）
            for(int cp = 0x30A0; cp <= 0x30FF; ++cp){
                glyphs.push_back(cp);
            }

            // 半角カナ（U+FF61 ～ U+FF9F）※必要であれば
            for(int cp = 0xFF61; cp <= 0xFF9F; ++cp){
                glyphs.push_back(cp);
            }
        ...
        static inline const ImWchar imJpGlyphRanges[] = {
        ...
        static inline std::vector<int> jpGlyph = CreateJpGlyphSet();
    };
}