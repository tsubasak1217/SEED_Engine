...
GameState_Play::GameState_Play(SEED::Scene_Base* pScene){
    // シーンの設定
    pScene_ = pScene;
}

GameState_Play::GameState_Play(SEED::Scene_Base* pScene, const SongInfo& songInfo, int32_t difficulty){
    // シーンの設定
    pScene_ = pScene;

    // 既存のオブジェクトの削除
    SEED::Hierarchy* hierarchy = pScene_->GetHierarchy();
