...
float CalcFocusLevel(float linearDepth) {
    float result;
    
    if (linearDepth >= min_max.x && linearDepth <= min_max.y) {
        result = (sin(4.71f + ((linearDepth - min_max.x) * inverseRange) * 6.28f) + 1.0f) * 0.5f;
    } else {
        result = 0.0f;
    }
