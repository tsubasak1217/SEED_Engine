...
namespace SEED{
    ...
    struct GuizmoInfo{
        ...
        Matrix4x4 parentMat = Methods::Matrix::IdentityMat4(); // 親の行列
    };
    ...
    public:
        ...
        //===================================================================================
        static void RegisterGuizmoItem(Transform* transform, const Matrix4x4& parentMat = Methods::Matrix::IdentityMat4()){
        ...
        static void RegisterGuizmoItem(Transform2D* transform, const Matrix4x4& parentMat = Methods::Matrix::IdentityMat4()){
            if(transform != nullptr){
                GuizmoInfo info;
    ...
    struct ImFunc{
        ...
        static bool Combo(const std::string& label, EnumType& currentValue, std::initializer_list<std::string> items, int padding = 0);
        template <typename EnumType>
        static bool Combo(const std::string& label, EnumType& currentValue, const char* const* items, int size, int padding = 0);
        template <typename EnumType>
        static bool ComboPair(const std::string& label, EnumType& currentValue, std::initializer_list<std::pair<std::string, EnumType>>items);
        static bool ComboText(const std::string& label, std::string& str, const std::vector<std::string>& items);
        // ラジオボタンの拡張関数
        template <typename EnumType>
        static bool RadioButton(const std::string& label, EnumType& currentValue, std::initializer_list<std::string> items, int padding = 0);
        template <typename EnumType>
        ...
        // ビットマスク
        template <typename EnumType>
        static bool BitMask(const std::string& label, EnumType& bit, std::initializer_list<std::string> bitNames);

        // inputTextに直接stringを渡せるように
        static bool InputTextMultiLine(const std::string& label, std::string& str, ImGuiInputTextFlags flags = 0, const ImVec2& size = ImVec2(0, 0));
    ...
    template<typename EnumType>
    inline bool ImFunc::Combo(const std::string& label, EnumType& currentValue, std::initializer_list<std::string> items, int padding){
    ...
    template<typename EnumType>
    inline bool ImFunc::ComboPair(const std::string& label, EnumType& currentValue, std::initializer_list<std::pair<std::string, EnumType>> items){
        // 一時的に const char* に変換
        std::vector<const char*> cstrItems;
        cstrItems.reserve(items.size());
    ...
    //////////////////////////////////////////////////////////////////
    template<typename EnumType>
    inline bool ImFunc::RadioButton(const std::string& label, EnumType& currentValue, std::initializer_list<std::string> items, int padding){
        bool changed = false;
    ...
    /////////////////////////////////////////////////////////////////
    template<typename EnumType>
    inline bool ImFunc::BitMask(const std::string& label, EnumType& bit, std::initializer_list<std::string> bitNames){
        bool changed = false;
