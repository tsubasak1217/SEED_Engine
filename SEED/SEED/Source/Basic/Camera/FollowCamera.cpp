...
namespace SEED{
    ...
    void FollowCamera::Initialize(){
        // カメラのオフセット初期設定
        defaultOffset_ = Methods::Math::Normalize(Vector3(0.0f, 2.0f, -5.0f));
        distance_ = 30.0f;
        // 角度の初期設定
        theta_ = -3.14f * 0.5f;
        phi_ = 3.14f * 0.45f;
    ...
    void FollowCamera::Update(){
        ...
        // カメラのオフセットベクトルを計算
        Vector3 offsetVec = Methods::SRT::CreateVector(theta_, phi_);
        // 目標カメラ位置の算出
        ...
        Vector3 eulerAngles = Methods::SRT::CalcRotateFromVec(Methods::Math::Normalize(targetPos_ - transform_.translate));
