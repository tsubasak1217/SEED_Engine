...
namespace SEED{
    void BlockSlideTransition::Update(){
        ...
        if(!isExitScene_){
            ...
            for(auto& quad : quads_){
                ...
                float ease = Methods::Easing::OutCubic(std::clamp(progress, 0.0f, 1.0f));
                quad.quad.translate = Methods::Math::Lerp(quad.startPos, quad.midPos, ease);
            }
            ...
            for(auto& quad : quads_){
                ...
                float ease = Methods::Easing::OutCubic(std::clamp(progress, 0.0f, 1.0f));
                quad.quad.translate = Methods::Math::Lerp(quad.midPos, quad.endPos, ease);
            }
        }
    }
    ...
    void BlockSlideTransition::CreateQuadInfo(){
        ...
        Topology::Quad2D q = Topology::Quad2D();
