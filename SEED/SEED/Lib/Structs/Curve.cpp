...
namespace SEED{
    ...
    float Curve::GetValue(float t, int channel) const{
        ...
        for(size_t i = 0; i < points.size() - 1; ++i){
            if(t >= points[i].x && t <= points[i + 1].x){
                ...
                float ease = Methods::Easing::Ease[curveType_](localT);
                return Methods::Math::Lerp(points[i].y, points[i + 1].y, ease);
            }
        }
    ...
    void Curve::FromJson(const nlohmann::json& json){
        ...
        // カーブタイプ
        curveType_ = static_cast<Methods::Easing::Type>(json.value("type", 0));
