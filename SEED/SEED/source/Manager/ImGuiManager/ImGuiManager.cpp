...
namespace SEED{
    ...
    void ImGuiManager::ExprolerMenu(){
        ...
        if(menuName_ == "ファイル/フォルダ"){
            ...
            if(ImGui::MenuItem("エクスプローラーで開く")){
                Methods::File::OpenInExplorer(explorerItemPath_);
            }

            // 削除
            if(ImGui::MenuItem("削除")){
                Methods::File::DeleteFileObject(explorerItemPath_);
            }

        } else if(menuName_ == "空選択"){
            // 新規作成
            if(ImGui::MenuItem("フォルダを作成")){
                Methods::File::CreateNewFolder(explorerItemPath_, "新規フォルダ");
            }
        }
    }
    ...
    ImVec2 ImFunc::SceneWindowBegin(const std::string& label, const std::string& cameraName, CustomWindowFlag customFlags, ImGuiWindowFlags normalFlags){
        ...
        {
            // static変数
            static std::unordered_map<ImGuiID, float> scales; // スケールを保存するマップ
    ...
    std::string ImFunc::FolderView(
        ...
        static std::unordered_map<std::string, std::string> focusedItems;
        ...
        if(ImGui::CollapsingHeader(label.c_str())){
            ...
            for(size_t i = 0; i < entries.size(); ++i){
                ...
                std::string name = Methods::String::ToString(entry.path().filename().u8string());
                ...
                // ドラッグ＆ドロップの開始
                std::string fullPathStr = Methods::String::ToString(entry.path().u8string());
                ...
                if(isLeftClicked){
                    ...
                    if(focusedItems[label] == id){
                        isEditName = true;
                        edittingName = Methods::String::ToString(entry.path().filename().u8string());
                ...
                if(!(isEditName && focusedItems[label] == id)){
                    ...
                    if(!isInputNameStartNow){
                        if(Input::GetIsActive()){
                            ...
                            // 変更
                            std::string newPath = Methods::String::ToString(entry.path().parent_path().u8string()) + "/" + newName;
                            Methods::File::RenameFile(Methods::String::ToString(entry.path().u8string()), newPath);
                        }
                    } else{
    ...
    bool ImFunc::InputText(const std::string& label, std::string& str){
    ...
    void ImFunc::Guizmo3D(const GuizmoInfo& info, ImDrawList* pDrawList, Range2D rectRange){
        ...
        Matrix4x4 deltaMat = Methods::Matrix::IdentityMat4(); // 変化量の行列
        ...
        // 変化量をトランスフォームに適用
        if(isManipulated){
            Matrix4x4 invParentMat = Methods::Matrix::InverseMatrix(info.parentMat);
    ...
    void ImFunc::Guizmo2D(const GuizmoInfo& info, ImDrawList* pDrawList, Range2D rectRange){
        ...
        // 必要な行列の用意
        BaseCamera* camera = SEED::Instance::GetCamera("debug");
        Matrix4x4 viewMat = Methods::Matrix::TranslateMatrix(Vector3(0.0f, 0.0f, camera->GetZNear() + 1.0f));
