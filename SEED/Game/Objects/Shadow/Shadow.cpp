...
void Shadow::Initialize(){
    ...
    model_->scale_ = {3.5f,3.5f,0.5f};
    //model_->blendMode_ = BlendMode::SUBTRACT;
}

static const Vector3 blockSize = {2.5f,2.5f,2.5f};
...
void Shadow::Update(Stage* currentStage){
    ...
    for(auto& object : currentStage->GetObjects()){
        ...
        if(hostPos.y >= objectTranslate.y){
            continue;
        }
        ...
        if(MyMath::LengthSq(nearestPos - hostPosXZ) <= 3.0f/*仮決め*/){
            // 範囲内なら
            // 一番高さが近いオブジェクトの位置を取得
