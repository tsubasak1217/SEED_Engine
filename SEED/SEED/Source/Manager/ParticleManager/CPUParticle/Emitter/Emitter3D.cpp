...
namespace SEED{
    ...
    void Emitter3D::Edit(){
        ...
        if(useGuizmo_){
            ...
            // 親グループの位置を考慮する
            Matrix4x4 parentMat = Methods::Matrix::IdentityMat4();
            if(parentGroup_){
                if(auto group3D = dynamic_cast<EmitterGroup3D*>(parentGroup_)){
                    parentMat = Methods::Matrix::TranslateMatrix(group3D->GetPosition());
                }
            }
        ...
        if(ImFunc::CollapsingHeader("回転" + idTag_)){
            ...
            if(isUseRotate_){
                ...
                if(useRotateDirection_){
                    ...
                    rotateDirection_ = Methods::Math::Normalize(rotateDirection_);
                }
                // 回転速度範囲の設定
