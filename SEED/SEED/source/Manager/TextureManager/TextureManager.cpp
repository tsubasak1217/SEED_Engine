...
namespace SEED{
    ...
    uint32_t TextureManager::CreateTexture(const std::string& filename, const aiTexture* embeddedTexture){
        ...
        if(!embeddedTexture){
            mipImages = Methods::DxFunc::LoadTextureImage(filename);// 通常のテクスチャ
        } else{
            mipImages = Methods::DxFunc::LoadEmbeddedTextureImage(embeddedTexture);// 埋め込みテクスチャ
        }

        // 作成
        const DirectX::TexMetadata& metadata = mipImages.GetMetadata();
        textureResources.push_back(Methods::DxFunc::CreateTextureResource(DxManager::GetInstance()->GetDevice(), metadata));
        textureResources.back()->SetName(Methods::String::ConvertString(filename).c_str());

        // 転送
        intermediateResources.push_back(
            Methods::DxFunc::UploadTextureData(
