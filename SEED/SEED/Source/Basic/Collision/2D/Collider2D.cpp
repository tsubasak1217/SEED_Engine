...
namespace SEED{
    ...
    void Collider2D::UpdateMatrix(){
        ...
        // ローカル行列の更新
        localMat_ = Methods::Matrix::AffineMatrix(scale_, rotate_, translate_);
        ...
        if(parentMat_){
            ...
            if(isParentRotate_ + isParentScale_ + isParentTranslate_ == 3){
                ...
                if(!isParentScale_){
                    Vector2 inverseScale = Vector2(1.0f, 1.0f) / Methods::Matrix::ExtractScale(*parentMat_);
                    cancelMat = cancelMat * Methods::Matrix::ScaleMatrix(inverseScale);
                }

                if(!isParentRotate_){
                    float inverseRotate = Methods::Matrix::ExtractRotation(*parentMat_) * -1.0f;
                    cancelMat = cancelMat * Methods::Matrix::RotateMatrix(inverseRotate);
                }

                if(!isParentTranslate_){
                    Vector2 inverseTranslate = Methods::Matrix::ExtractTranslation(*parentMat_) * -1.0f;
    ...
    bool Collider2D::CheckCollision(const Topology::Quad2D& quad){
        quad;
        return false;
    }

    bool Collider2D::CheckCollision(const Topology::Circle& circle){
        circle;
        return false;
    }
    ...
    void Collider2D::PushBack(Collider2D* collider1, Collider2D* collider2, CollisionData2D collisionData){
        ...
        if(parent[0]){
            ...
            if(Methods::Math::Dot(collisionData.hitNormal.value(), { 0.0f,-1.0f }) > 0.7f){
                collider1->ownerObject_->SetIsOnGround(true);

            }
            // 天井判定
            else if(Methods::Math::Dot(collisionData.hitNormal.value(), { 0.0f,1.0f }) > 0.7f){
    ...
    void Collider2D::SetOwnerObject(GameObject2D* parentObject){
        ownerObject_ = parentObject;
        objectType_ = parentObject->GetObjectType();
    }
}