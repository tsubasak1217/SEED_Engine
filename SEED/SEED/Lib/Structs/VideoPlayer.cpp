...
namespace SEED{
    ...
    // ビデオのサイズ取得
    /////////////////////////////////////////////////////////////////////////////////////////////////
    Vector2 VideoPlayer::GetVideoSize() const{
        return Vector2(static_cast<float>(videoData_.widthY), static_cast<float>(videoData_.heightY));
    }

    //////////////////////////////////////////////////////////////////////////////////////////////////
    // ビデオのグラフハンドル取得
    //////////////////////////////////////////////////////////////////////////////////////////////////
    int32_t VideoPlayer::GetVideoGH() const{
        if(!videoItem_){ return -1; }
        return videoItem_->frameTextureRGBA_.GetSRVIndex();
    }
    ...
    void VideoPlayer::Draw(Topology::Quad2D quad){
        // 動画が読み込まれていない場合は何もしない
        if(!videoItem_){ return; }
        // Topology::Quadのテクスチャを設定
        quad.GH = videoItem_->frameTextureRGBA_.GetSRVIndex();
        // 描画
        SEED::Instance::DrawQuad2D(quad);
    }

    void VideoPlayer::Draw(Topology::Quad quad){
        // 動画が読み込まれていない場合は何もしない
        if(!videoItem_){ return; }
        // Topology::Quadのテクスチャを設定
        quad.GH = videoItem_->frameTextureRGBA_.GetSRVIndex();
