...
namespace SEED{
    ...
    void Octree::AddCollider(Collider* object){
        ...
        if(children_.empty()){
            ...
            children_.resize(8);
            children_[0] = std::make_unique<Octree>(Topology::AABB(center - halfSize, halfSize), depth_ + 1, maxDepth_, this);// 左下手前
            children_[1] = std::make_unique<Octree>(Topology::AABB(center + Vector3(halfSize.x, -halfSize.y, -halfSize.z), halfSize), depth_ + 1, maxDepth_, this);// 右下手前
            children_[2] = std::make_unique<Octree>(Topology::AABB(center + Vector3(-halfSize.x, halfSize.y, -halfSize.z), halfSize), depth_ + 1, maxDepth_, this);// 左上手前
            children_[3] = std::make_unique<Octree>(Topology::AABB(center + Vector3(halfSize.x, halfSize.y, -halfSize.z), halfSize), depth_ + 1, maxDepth_, this);// 右上手前
            children_[4] = std::make_unique<Octree>(Topology::AABB(center + Vector3(-halfSize.x, -halfSize.y, halfSize.z), halfSize), depth_ + 1, maxDepth_, this);// 左下奥
            children_[5] = std::make_unique<Octree>(Topology::AABB(center + Vector3(halfSize.x, -halfSize.y, halfSize.z), halfSize), depth_ + 1, maxDepth_, this);// 右下奥
            children_[6] = std::make_unique<Octree>(Topology::AABB(center + Vector3(-halfSize.x, halfSize.y, halfSize.z), halfSize), depth_ + 1, maxDepth_, this);// 左上奥
            children_[7] = std::make_unique<Octree>(Topology::AABB(center + halfSize, halfSize), depth_ + 1, maxDepth_, this);// 右上奥
