...
void PlayField::Initialize(){
    // ゲームカメラの取得
    SEED::BaseCamera* gameCamera_ =SEED::Instance::GetCamera("gameCamera");
    ...
    for(int i = 0; i < kKeyCount_; i++){
        ...
        // 画像を設定
        laneAnswer_[0][i].tri.GH = SEED::TextureManager::LoadTexture("PlayField/gradation.png");
        laneAnswer_[1][i].tri.GH = SEED::TextureManager::LoadTexture("PlayField/gradation.png");
        laneAnswer_[0][i].evaluationPolygon.GH = SEED::TextureManager::LoadTexture("PlayField/gradation.png");
        laneAnswer_[1][i].evaluationPolygon.GH = SEED::TextureManager::LoadTexture("PlayField/gradation.png");
        laneBorderLine_[0][i].GH = SEED::TextureManager::LoadTexture("PlayField/borderLine.png");
        laneBorderLine_[1][i].GH = SEED::TextureManager::LoadTexture("PlayField/borderLine.png");
        laneBorderLineAura_[0][i].GH = SEED::TextureManager::LoadTexture("PlayField/lineAura.png");
        laneBorderLineAura_[1][i].GH = SEED::TextureManager::LoadTexture("PlayField/lineAura.png");

        // blendModeを設定
        laneAnswer_[0][i].tri.blendMode = SEED::BlendMode::ADD;
        laneAnswer_[1][i].tri.blendMode = SEED::BlendMode::ADD;
        laneAnswer_[0][i].evaluationPolygon.blendMode = SEED::BlendMode::ADD;
        laneAnswer_[1][i].evaluationPolygon.blendMode = SEED::BlendMode::ADD;
        laneBorderLine_[0][i].blendMode = SEED::BlendMode::ADD;
        laneBorderLine_[1][i].blendMode = SEED::BlendMode::ADD;
        laneBorderLineAura_[0][i].blendMode = SEED::BlendMode::ADD;
        laneBorderLineAura_[1][i].blendMode = SEED::BlendMode::ADD;
        ...
        if(i == kKeyCount_ - 1){
            ...
            laneBorderLine_[0][i + 1].GH = SEED::TextureManager::LoadTexture("PlayField/borderLine.png");
            laneBorderLine_[1][i + 1].GH = SEED::TextureManager::LoadTexture("PlayField/borderLine.png");
            laneBorderLineAura_[0][i + 1].GH = SEED::TextureManager::LoadTexture("PlayField/lineAura.png");
            laneBorderLineAura_[1][i + 1].GH = SEED::TextureManager::LoadTexture("PlayField/lineAura.png");
            laneBorderLine_[0][i + 1].blendMode = SEED::BlendMode::ADD;
            laneBorderLine_[1][i + 1].blendMode = SEED::BlendMode::ADD;
            laneBorderLineAura_[0][i + 1].blendMode = SEED::BlendMode::ADD;
            laneBorderLineAura_[1][i + 1].blendMode = SEED::BlendMode::ADD;
        }
    }
    ...
    SEED::GameObject* backEffect = hierarchy->LoadObject("PlayScene/Effects/StageBack.prefab");
    ...
    for(int32_t i = 0; i < laneEffectObjects_.size(); i++){
        ...
        for(int32_t j = 0; j < 3; j++){
            laneEffectObjects_[i]->GetComponent<SEED::Component_EmitterGroup3D>(j)->Deactivate();
        }
    }

    /*ホイール*/
    wheelEffectObjects_[(int)SEED::GeneralEnum::UpDown::UP] = hierarchy->LoadObject("PlayScene/Effects/Wheel_UP.prefab");
    wheelEffectObjects_[(int)SEED::GeneralEnum::UpDown::DOWN] = hierarchy->LoadObject("PlayScene/Effects/Wheel_DOWN.prefab");

    for(int32_t i = 0; i < wheelEffectObjects_.size(); i++){
        // 最初は非アクティブにしておく
        wheelEffectObjects_[i]->GetComponent<SEED::Component_EmitterGroup3D>(0)->Deactivate();
    }
    ...
    for(int32_t i = 0; i < rectFlickEffectObjects_.size(); i++){
        // 最初は非アクティブにしておく
        rectFlickEffectObjects_[i]->GetComponent<SEED::Component_EmitterGroup2D>(0)->Deactivate();
    }
    ...
    // エフェクトオブジェクト名の設定
    fastLateObjNames_[(int)SEED::GeneralEnum::UpDown::UP][(int)Timing::Fast] = "PlayScene/Effects/fast_up.prefab";
    fastLateObjNames_[(int)SEED::GeneralEnum::UpDown::UP][(int)Timing::Late] = "PlayScene/Effects/late_up.prefab";
    fastLateObjNames_[(int)SEED::GeneralEnum::UpDown::DOWN][(int)Timing::Fast] = "PlayScene/Effects/fast_down.prefab";
    fastLateObjNames_[(int)SEED::GeneralEnum::UpDown::DOWN][(int)Timing::Late] = "PlayScene/Effects/late_down.prefab";

    // 背景色の設定
    bgColorObj_ = hierarchy->LoadObject("PlayScene/BGColorObj.prefab");
   SEED::Instance::SetWindowColor(bgColorObj_->GetComponent<SEED::ColorListComponent>()->GetColor(0));
}
...
// レーンの押下状態を設定
/////////////////////////////////////////////////////////////////////////
void PlayField::SetEvaluation(LaneBit laneBit, SEED::GeneralEnum::UpDown layer, const SEED::Color& color){
...
void PlayField::SetLanePressed(int32_t lane, const SEED::Color& color){
    // レーンの押下状態を設定
...
/////////////////////////////////////////////////////////////////////////
SEED::Topology::Quad PlayField::GetNoteQuad(float timeRatio, int32_t lane, SEED::GeneralEnum::UpDown layer, float ratioWidth){
    SEED::Topology::Quad result;
    SEED::Topology::Triangle laneTriangle = lane_[(int)layer][lane];
    ...
    // ノーツの矩形の頂点を計算
    result.localVertex[0] = SEED::Methods::Math::Lerp(laneTriangle.localVertex[0], laneTriangle.localVertex[2], 1.0f - farZ);
    result.localVertex[1] = SEED::Methods::Math::Lerp(laneTriangle.localVertex[0], laneTriangle.localVertex[1], 1.0f - farZ);
    result.localVertex[2] = SEED::Methods::Math::Lerp(laneTriangle.localVertex[0], laneTriangle.localVertex[2], 1.0f - nearZ);
    result.localVertex[3] = SEED::Methods::Math::Lerp(laneTriangle.localVertex[0], laneTriangle.localVertex[1], 1.0f - nearZ);

    return result;
}

SEED::Topology::Quad PlayField::GetWheelFloorQuad(float timeRatio, SEED::GeneralEnum::UpDown layer, float ratioWidth){
    SEED::Topology::Quad result;
    ...
    if(layer == SEED::GeneralEnum::UpDown::UP){
        result.localVertex[0] = SEED::Methods::Math::Lerp(playFieldPointsWorld_[TOP], playFieldPointsWorld_[LEFT], farZ);
        result.localVertex[1] = SEED::Methods::Math::Lerp(playFieldPointsWorld_[TOP], playFieldPointsWorld_[RIGHT], farZ);
        result.localVertex[2] = SEED::Methods::Math::Lerp(playFieldPointsWorld_[TOP], playFieldPointsWorld_[LEFT], nearZ);
        result.localVertex[3] = SEED::Methods::Math::Lerp(playFieldPointsWorld_[TOP], playFieldPointsWorld_[RIGHT], nearZ);
    } else{
        result.localVertex[0] = SEED::Methods::Math::Lerp(playFieldPointsWorld_[BOTTOM], playFieldPointsWorld_[RIGHT], farZ);
        result.localVertex[1] = SEED::Methods::Math::Lerp(playFieldPointsWorld_[BOTTOM], playFieldPointsWorld_[LEFT], farZ);
        result.localVertex[2] = SEED::Methods::Math::Lerp(playFieldPointsWorld_[BOTTOM], playFieldPointsWorld_[RIGHT], nearZ);
        result.localVertex[3] = SEED::Methods::Math::Lerp(playFieldPointsWorld_[BOTTOM], playFieldPointsWorld_[LEFT], nearZ);
    }
    return result;
}

SEED::Topology::Quad PlayField::GetWheelDirectionQuad(float timeRatio, SEED::GeneralEnum::UpDown layer){
    SEED::Topology::Quad result;
    ...
    // ノーツの矩形の頂点を計算
    if(layer == SEED::GeneralEnum::UpDown::UP){
        result.localVertex[2] = SEED::Methods::Math::Lerp(playFieldPointsWorld_[TOP], playFieldPointsWorld_[LEFT], timeRatio);
        result.localVertex[3] = SEED::Methods::Math::Lerp(playFieldPointsWorld_[TOP], playFieldPointsWorld_[RIGHT], timeRatio);
        ...
        result.localVertex[2] = SEED::Methods::Math::Lerp(playFieldPointsWorld_[BOTTOM], playFieldPointsWorld_[RIGHT], timeRatio);
        result.localVertex[3] = SEED::Methods::Math::Lerp(playFieldPointsWorld_[BOTTOM], playFieldPointsWorld_[LEFT], timeRatio);
        result.localVertex[0] = result.localVertex[2] + Vector3(0.0f, -height, 0.0f);
        result.localVertex[1] = result.localVertex[3] + Vector3(0.0f, -height, 0.0f);
    }
    return result;
...
// RectFlickの矩形を取得
SEED::Topology::Quad2D PlayField::GetRectFlickQuad(float timeRatio, SEED::GeneralEnum::DIRECTION8 dir, float ratioWidth){
    SEED::Topology::Quad2D result;
    Vector2 points[2];
    ...
    // ノーツの矩形の頂点を計算
    result.localVertex[0] = SEED::Methods::Math::Lerp(kWindowCenter, points[0], farZ);
    result.localVertex[1] = SEED::Methods::Math::Lerp(kWindowCenter, points[1], farZ);
    result.localVertex[2] = SEED::Methods::Math::Lerp(kWindowCenter, points[0], nearZ);
    result.localVertex[3] = SEED::Methods::Math::Lerp(kWindowCenter, points[1], nearZ);
...
// レーンのエフェクトを発生させる関数
/////////////////////////////////////////////////////////////
void PlayField::LaneEffect(int evaluation, LaneBit laneBit, SEED::GeneralEnum::UpDown layer, Timing timing){
    ...
    default:// それ以外のとき
        laneEffectObjects_[laneIndex]->GetComponent<SEED::Component_EmitterGroup3D>(evaluation)->Activate();
        laneEffectObjects_[laneIndex]->GetComponent<SEED::Component_EmitterGroup3D>(evaluation)->InitEmitters();
        break;
    }
...
/////////////////////////////////////////////////////////////
void PlayField::WheelEffect(int evaluation, LaneBit laneBit, SEED::GeneralEnum::UpDown layer, Timing timing){

    SEED::GeneralEnum::UpDown dir;
    if(laneBit & LaneBit::WHEEL_UP){
        dir = SEED::GeneralEnum::UpDown::UP;
    } else{
        dir = SEED::GeneralEnum::UpDown::DOWN;
    }
    ...
    default:// それ以外のとき
        ...
        wheelEffectObjects_[int(dir)]->GetComponent<SEED::Component_EmitterGroup3D>()->Activate();
        wheelEffectObjects_[int(dir)]->GetComponent<SEED::Component_EmitterGroup3D>()->InitEmitters();
    }
...
void PlayField::RectFlickEffect(int evaluation, LaneBit laneBit){
    ...
    if(laneBit & LaneBit::RECTFLICK_LT){
        rectFlickEffectObjects_[0]->GetComponent<SEED::Component_EmitterGroup2D>()->Activate();
        rectFlickEffectObjects_[0]->GetComponent<SEED::Component_EmitterGroup2D>()->InitEmitters();
    }
    if(laneBit & LaneBit::RECTFLICK_RT){
        rectFlickEffectObjects_[1]->GetComponent<SEED::Component_EmitterGroup2D>()->Activate();
        rectFlickEffectObjects_[1]->GetComponent<SEED::Component_EmitterGroup2D>()->InitEmitters();
    }
    if(laneBit & LaneBit::RECTFLICK_LB){
        rectFlickEffectObjects_[2]->GetComponent<SEED::Component_EmitterGroup2D>()->Activate();
        rectFlickEffectObjects_[2]->GetComponent<SEED::Component_EmitterGroup2D>()->InitEmitters();
    }
    if(laneBit & LaneBit::RECTFLICK_RB){
        rectFlickEffectObjects_[3]->GetComponent<SEED::Component_EmitterGroup2D>()->Activate();
        rectFlickEffectObjects_[3]->GetComponent<SEED::Component_EmitterGroup2D>()->InitEmitters();
    }
}
...
// エフェクトを発生させる関数
/////////////////////////////////////////////////////////////
void PlayField::EmitEffect(LaneBit laneBit, SEED::GeneralEnum::UpDown layer, int evaluation, Timing timing){
...
Vector3 PlayField::GetCursorWorldPos(float cursorX){
    ...
    Vector3 result = SEED::Methods::Math::Lerp(playFieldPointsWorld_[LEFT], playFieldPointsWorld_[RIGHT], t);
    return result;
}
