...
namespace SEED{
    ...
    void Model::Initialize(const std::string& filename){
        ...
        worldMat_ = Methods::Matrix::IdentityMat4();
        ...
        for(int i = 0; i < modelData->materials.size(); i++){
            ...
            newMaterial.uvTransform = Methods::Matrix::AffineMatrix(
                material->UV_scale_,
                { 0.0f,0.0f,0.0f },
    ...
    void Model::UpdateMatrix(){
        ...
        // ローカル変換行列の更新
        localMat_ = Methods::Matrix::AffineMatrix(transform_.scale, transform_.rotate, transform_.translate);
            ...
            if(progressOfAnimLerp_ < 1.0f){
                ...
                std::for_each(std::execution::par_unseq, animetedSkeleton_->joints.begin(), animetedSkeleton_->joints.end(), [&](const ModelJoint& joint){
                    ...
                    palette_[jointIndex].skeletonSpaceInverceTransposeMatrix =
                        Methods::Matrix::Transpose(Methods::Matrix::InverseMatrix(palette_[jointIndex].skeletonSpaceMatrix));
                });
