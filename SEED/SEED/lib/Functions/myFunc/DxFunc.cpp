...
ModelData LoadObjFile(const std::string& directoryPath, const std::string& filename){
    ...
    while(std::getline(file, line)){
        ...
        if(identifer == "o"){// 新しいメッシュ-----------------------------
            ...
            //positions.push_back(std::vector<Vector4>());
            //normals.push_back(std::vector<Vector3>());
            //texcoords.push_back(std::vector<Vector2>());
            ...
            // mtlファイルを読み込む
            modelData.materialData = LoadMaterialTemplateFile(directoryPath + "/" + filename, materialFilename);
        }
    }
