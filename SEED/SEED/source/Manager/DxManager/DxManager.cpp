...
namespace SEED{
    ...
    void DxManager::CreateDevice(){
        ...
        for(
            ...
            if(adapterDesc.Flags != DXGI_ADAPTER_FLAG3_SOFTWARE){
                ...
                // 使用アダプタを出力してループ終了
                Methods::DxFunc::Log(std::format(L"useAdapter:{}\n", adapterDesc.Description));
                break;
        ...
        for(size_t i = 0; i < _countof(featureLevels); ++i){
            ...
            if(SUCCEEDED(hr)){
                Methods::DxFunc::Log(std::format("FeatureLevel : {}\n", featureLevelString[i]));
                break;
            }
        }
        ...
        // 初期化成功をログに表示
        Methods::DxFunc::Log("Complete create D3D12Device!!!\n");
    }
    ...
    void DxManager::CreateRenderResource(const std::string& cameraName){
        ...
        {
            // レンダーターゲットとして作成
            SEED::DxResource& offScreenResource = offScreenResources[cameraName];
            std::string resourceName = "offScreen_" + cameraName;
            offScreenResource.resource = Methods::DxFunc::CreateRenderTargetTextureResource(device.Get(), pSEED_->kClientWidth_, pSEED_->kClientHeight_);
            offScreenResource.resource.Get()->SetName(Methods::String::ConvertString(resourceName).c_str());
        ...
        }
        {
            ...
            depthStencilResource.resource = Methods::DxFunc::CreateDepthStencilTextureResource(device.Get(), pSEED_->kClientWidth_, pSEED_->kClientHeight_);
            depthStencilResource.resource->SetName(Methods::String::ConvertString(resourceName).c_str());
    ...
    void DxManager::ClearViewSettings(){
        ...
        for(const auto& [cameraName, offScreenHandle] : offScreenHandles){
            // オフスクリーンのRTVをクリアする
            clearColor = Methods::Math::FloatColor(SEED::Instance::GetWindowColor(), false);
