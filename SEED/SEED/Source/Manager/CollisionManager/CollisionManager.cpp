...
namespace SEED{
    ...
    void CollisionManager::Initialize(){
        ...
        instance_->octree_ = std::make_unique<Octree>(
            Topology::AABB(Vector3(0.0f, 0.0f, 0.0f), Vector3(2000.0f, 2000.0f, 2000.0f)),
            0, 5, nullptr
        );
    ...
    void CollisionManager::CheckCollision(){
        ...
        {
            for(auto& collider : instance_->colliderList2D_){
                SEED::GameObject2D* owner = collider.second->GetOwnerObject();
                if(owner){
    ...
    void CollisionManager::ResetOctree(const Topology::AABB& range, int32_t depth){
