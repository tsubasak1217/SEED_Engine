...
namespace SEED{
    namespace Methods{
        namespace Shape{
            ...
            Topology::Quad MakeEqualQuad(float radius, const Color& color){
                return Topology::Quad(
                    { -radius, radius ,0.0f },
                    { radius, radius,0.0f },
                    { -radius, -radius ,0.0f },
                    { radius, -radius,0.0f },
            ...
            Topology::Quad2D MakeEqualQuad2D(float radius, const Color& color){
                return Topology::Quad2D(
                    { -radius, -radius },
                    { radius, -radius },
                    { -radius, radius },
                    { radius, radius },
            ...
            Topology::Quad2D MakeBackgroundQuad2D(int32_t layer, const Color& color){
                Topology::Quad2D quad = MakeQuad2D(kWindowSize, color);
                for(int i = 0; i < 4; ++i){
            ...
            Topology::Quad2D MakeFrontQuad2D(int32_t layer, const Color& color){
                Topology::Quad2D quad = MakeQuad2D(kWindowSize, color);
                for(int i = 0; i < 4; ++i){
                    quad.localVertex[i].x += kWindowSize.x * 0.5f;
                    quad.localVertex[i].y += kWindowSize.y * 0.5f;
            ...
            Topology::Quad MakeQuad(const Vector2& size, const Color& color, const Vector2& anchorPoint){

                Topology::Quad quad = Topology::Quad(
                    { 0.0f, 0.0f, 0.0f },
                    { size.x, 0.0f, 0.0f },
                    { 0.0f, -size.y, 0.0f },
                    { size.x, -size.y, 0.0f },
            ...
            Topology::Quad2D MakeQuad2D(const Vector2& size, const Color& color, const Vector2& anchorPoint){
                Topology::Quad2D quad = Topology::Quad2D(
                    { 0.0f, 0.0f },
                    { size.x, 0.0f },
                    { 0.0f, size.y },
                    { size.x, size.y },
            ...
            std::array<Vector3, 2> LineClosestPoints(const Topology::Line& l1, const Topology::Line& l2){
                ...
                // 2直線の長さ
                float length1 = Math::Length(dir1);
                float length2 = Math::Length(dir2);
                ...
                if(length1 == 0.0f){
                    if(length2 == 0.0f){
                        ...
                        return { l1.origin_,Math::ClosestPoint(l2.origin_,l2.end_,l1.origin_) };
                    }
                } else if(length2 == 0.0f){
                    if(length1 == 0.0f){
                        return { l1.origin_,l2.origin_ };
                    } else{
                        return { Math::ClosestPoint(l1.origin_,l1.end_,l2.origin_),l2.origin_ };
                    }
                }
                ...
                // 2直線の方向ベクトルの外積
                Vector3 cross = Math::Cross(dir1, dir2);
                ...
                float t1 = Math::Dot(Math::Cross(originVec, dir2), cross) / crossLengthSq;
                float t2 = Math::Dot(Math::Cross(originVec, dir1), cross) / crossLengthSq;
            ...
            float LineDistance(const Topology::Line& l1, const Topology::Line& l2){
                ...
                return Math::Length(closest[1] - closest[0]);
            }
            ...
            Topology::AABB MaxAABB(const Topology::AABB& aabb1, const Topology::AABB& aabb2){
                ...
                return Topology::AABB(mostMin + halfSize, halfSize);
            }
            ...
            Topology::AABB2D MaxAABB(const Topology::AABB2D& aabb1, const Topology::AABB2D& aabb2){
                ...
                return Topology::AABB2D(mostMin + halfSize, halfSize);
            }
