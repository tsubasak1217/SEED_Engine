...
public:
    ...
    const Vector3& GetLocalScale() const{ return model_->transform_.scale; }
    void SetScale(const Vector3& scale){ model_->transform_.scale = scale; }
    void SetScaleX(float x){ model_->transform_.scale.x = x; }
    void SetScaleY(float y){ model_->transform_.scale.y = y; }
    void SetScaleZ(float z){ model_->transform_.scale.z = z; }
    /*------ rotate -------*/
    Vector3 GetWorldRotate() const{ return model_->GetWorldRotate(); }
    const Vector3& GetLocalRotate() const{ return model_->transform_.rotate; }
    void SetRotateWithQuaternion(bool isRotateWithQuaternion){ model_->isRotateWithQuaternion_ = isRotateWithQuaternion; }
    void SetRotate(const Vector3& rotate){ model_->transform_.rotate = rotate; }
    void SetRotate(const Quaternion& rotate){ model_->transform_.rotateQuat = rotate; }
    void SetRotateX(float x){ model_->transform_.rotate.x = x; }
    void SetRotateY(float y){ model_->transform_.rotate.y = y; }
    void SetRotateZ(float z){ model_->transform_.rotate.z = z; }
    /*------ translate -------*/
    Vector3 GetWorldTranslate() const{ return model_->GetWorldTranslate(); }
    const Vector3& GetLocalTranslate() const{ return model_->transform_.translate; }
    void AddWorldTranslate(const Vector3& addValue);
    const Vector3& GetPrePos() const{ return prePos_; }
    void SetTranslate(const Vector3& translate){ model_->transform_.translate = translate; }
    void SetTranslateX(float x){ model_->transform_.translate.x = x; }
    void SetTranslateY(float y){ model_->transform_.translate.y = y; }
    void SetTranslateZ(float z){ model_->transform_.translate.z = z; }
    void AddTranslate(const Vector3& addValue){ model_->transform_.translate += addValue; }
    /*------ matrix -------*/
