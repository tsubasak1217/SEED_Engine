...
// IMGUIデバッグ用
//////////////////////////////////////////////////////////////////////////
void StageViewCamera::ShowGui(){
    // 現在のピッチ・ヨーを度数で表示
    ImGui::Text("pitch (deg): %.1f", pitch_);
    ImGui::Text("yaw   (deg): %.1f", yaw_);

    // パラメータ操作
    ImGui::DragFloat("rotationSpeed", &rotationSpeed_, 0.01f);
    ImGui::DragFloat("radius", &radius_, 0.1f, 0.0f, 1000.0f);

    ImGui::DragFloat("minPitchDeg", &minPitchDeg_, 0.1f, 0.0f, 89.9f);
    ImGui::DragFloat("maxPitchDeg", &maxPitchDeg_, 0.1f, 0.0f, 89.9f);
}
