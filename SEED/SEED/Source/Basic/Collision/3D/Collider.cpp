...
namespace SEED{
    ...
    void Collider::UpdateMatrix(){
        ...
        if(parentMat_){
            ...
            if(isParentRotate_ + isParentScale_ + isParentTranslate_ == 3){
                ...
                if(!isParentScale_){
                    Vector3 inverseScale = Vector3(1.0f, 1.0f, 1.0f) / Methods::Matrix::ExtractScale(*parentMat_);
                    cancelMat = cancelMat * Methods::Matrix::ScaleMatrix(inverseScale);
                }

                if(!isParentRotate_){
                    Vector3 inverseRotate = Methods::Matrix::ExtractRotation(*parentMat_) * -1.0f;
                    cancelMat = cancelMat * Methods::Matrix::RotateMatrix(inverseRotate);
                }

                if(!isParentTranslate_){
                    Vector3 inverseTranslate = Methods::Matrix::ExtractTranslation(*parentMat_) * -1.0f;
    ...
    bool Collider::CheckCollision(const Topology::AABB& aabb){
        aabb;
        return false;
    }

    bool Collider::CheckCollision(const Topology::OBB& obb){
        obb;
        return false;
    }

    bool Collider::CheckCollision(const Topology::Line& line){
        line;
        return false;
    }

    bool Collider::CheckCollision(const Topology::Sphere& sphere){
        sphere;
        return false;
    }
    ...
    void Collider::PushBack(Collider* collider1, Collider* collider2, CollisionData collisionData){
        ...
        SEED::GameObject* parent[2] = { collider1->GetOwnerObject(),collider2->GetOwnerObject() };
        ...
        if(parent[0]){
            ...
            if(Methods::Math::Dot(collisionData.hitNormal.value(), { 0.0f,1.0f,0.0f }) > 0.7f){
