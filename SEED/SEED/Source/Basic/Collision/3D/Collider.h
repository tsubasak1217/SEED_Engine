...
namespace SEED{
    ...
    public:
        ...
        virtual bool CheckCollision(const Topology::AABB& aabb);
        virtual bool CheckCollision(const Topology::OBB& obb);
        virtual bool CheckCollision(const Topology::Line& line);
        virtual bool CheckCollision(const Topology::Sphere& sphere);
    ...
    public:// アクセッサ--------------------------------------------------------------
        ...
        // トランスフォーム取得
        Vector3 GetWoarldTranslate()const{ return Methods::Matrix::ExtractTranslation(worldMat_) + offset_; }
        Vector3 GetLocalTranslate()const{ return Methods::Matrix::ExtractTranslation(localMat_) + offset_; }
        void AddTranslate(const Vector3& addValue){ translate_ += addValue; }
        Vector3 GetWorldRotate()const{ return Methods::Matrix::ExtractRotation(worldMat_); }
        Vector3 GetLocalRotate()const{ return Methods::Matrix::ExtractRotation(localMat_); }
        Vector3 GetWorldScale()const{ return Methods::Matrix::ExtractScale(worldMat_); }
        Vector3 GetLocalScale()const{ return Methods::Matrix::ExtractScale(localMat_); }
        virtual bool IsMoved();
        const Vector3& GetCollidedPosition()const{ return collidedPosition_; }

        // 衝突判定八分木用
        const Topology::AABB& GetBox()const{ return coverAABB_; }
    ...
    protected:// 衝突に使用するパラメータ--------------------------------------------------
        Topology::AABB coverAABB_;// 頂点を包含するAABB
