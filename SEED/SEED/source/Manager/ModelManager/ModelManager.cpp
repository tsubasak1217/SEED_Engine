...
namespace SEED{
        ...
        for(uint32_t materialIdx = 0; materialIdx < scene->mNumMaterials; ++materialIdx){
            ...
            if(material->GetTexture(aiTextureType_DIFFUSE, 0, &texturePath) == AI_SUCCESS){
                ...
                if(textureFilePath.find("*") == std::string::npos){
                    ...
                    // まずmodel階層内から探す
                    findResult = Methods::File::FindFile("Resources/models/", texturePath.C_Str());
                    if(!findResult.empty()){
                        materialData.textureFilePath_ = "../models/" + findResult.string();

                    } else{// 見つからなかったらTextures階層から探す
                        findResult = Methods::File::FindFile("Resources/Textures/", texturePath.C_Str());
                        if(!findResult.empty()){
    ...
    ModelNode ModelManager::ReadModelNode(const aiNode* node){
        ...
        result.localMatrix = Methods::Matrix::AffineMatrix(result.transform.scale, result.transform.rotate, result.transform.translate);
    ...
    int32_t ModelManager::CreateJoint(
        ...
        joint.skeletonSpaceMatrix = Methods::Matrix::IdentityMat4();
    ...
    void ModelManager::AnimatedSkeleton(
        ...
        std::for_each(std::execution::par_unseq, defaultSkeleton.joints.begin(), defaultSkeleton.joints.end(), [&](auto& defaultJoint){
            ...
            if(auto it = modelAnimation.nodeAnimations.find(defaultJoint.name); it != modelAnimation.nodeAnimations.end()){
                ...
                pSkeleton->joints[idx].skeletonSpaceMatrix = Methods::Matrix::IdentityMat4();
            }
    ...
    ModelSkeleton ModelManager::InterpolateSkeleton(
        ...
        for(uint32_t i = 0; i < skeletonA.joints.size(); ++i){
            ...
            Vector3 translate = Methods::Math::Lerp(jointA.transform.translate, jointB.transform.translate, t);
            Vector3 scale = Methods::Math::Lerp(jointA.transform.scale, jointB.transform.scale, t);
            ...
            joint.localMatrix = Methods::Matrix::AffineMatrix(scale, rotate, translate);
    ...
    void ModelManager::UpdateSkeleton(ModelSkeleton* skeleton){
        ...
        std::for_each(std::execution::par_unseq, skeleton->joints.begin(), skeleton->joints.end(), [&](ModelJoint& joint){
            // ローカル行列を更新
            joint.localMatrix = Methods::Matrix::AffineMatrix(joint.transform.scale, joint.transform.rotate, joint.transform.translate);
        });
        ...
        for(uint32_t meshIdx = 0; meshIdx < scene->mNumMeshes; ++meshIdx){
            ...
            if(mesh->mNumBones == 0){
                ...
                if(meshNode){
                    ...
                    Matrix4x4 bindPoseMatrix = Methods::Matrix::AffineMatrix(
                        { scale.x, scale.y, scale.z },
                        { rotate.x, -rotate.y, -rotate.z, rotate.w },
                        { -translate.x, translate.y, translate.z }
                    );
                    jointWeightData.inverseBindPoseMatrix = Methods::Matrix::InverseMatrix(bindPoseMatrix);
    ...
    void ModelManager::CreateVertexInfluence(const ModelSkeleton& skeleton, ModelData* modelData){
        ...
        palette.inverseBindPoseMatrices.resize(skeleton.joints.size(), Methods::Matrix::IdentityMat4());
