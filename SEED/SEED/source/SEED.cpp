...
namespace SEED{
    ...
    void Instance::SetImGuiEmptyWindows(){
    #ifdef _DEBUG
    ...
    void Instance::DrawGUI(){
        ...
        if(ImGui::Checkbox("デバッグカメラ", &isDebugCamera_)){
            if(isDebugCamera_){
                Instance::SetMainCamera("debug");
                Instance::SetIsCameraActive("debug", true);
            } else{
                Instance::SetMainCamera("default");
                Instance::SetIsCameraActive("debug", false);
            }
        }
        ...
        if(ImGui::ColorEdit4("ウインドウの背景色", (float*)&instance_->clearColor_)){
            // ウインドウの背景色を変更
            instance_->windowBackColor_ = Methods::Math::IntColor(instance_->clearColor_);
        }
    ...
    void Instance::Initialize(int clientWidth, int clientHeight, HINSTANCE hInstance, int nCmdShow){
        ...
        instance_->offscreenWrapper_->color = Methods::Math::FloatColor(0, 0, 0, 256);
        ...
        // Vec4版の色の計算
        instance_->clearColor_ = Methods::Math::FloatColor(windowBackColor_);
    }
    ...
    void Instance::DrawTriangle(const Topology::Triangle& triangle){
        Matrix4x4 worldMat = Methods::Matrix::AffineMatrix(triangle.scale, triangle.rotate, triangle.translate);
        int GH = triangle.GH;
        ...
        instance_->pPolygonManager_->AddTriangle(
            Methods::Matrix::TransformToVec4(triangle.localVertex[0]),
            Methods::Matrix::TransformToVec4(triangle.localVertex[1]),
            Methods::Matrix::TransformToVec4(triangle.localVertex[2]),
            worldMat, triangle.color, triangle.lightingType, triangle.uvTransform, true,
            true, GH, triangle.blendMode, triangle.cullMode
        );
    }

    void Instance::AddTriangle3DPrimitive(
    ...
    void Instance::DrawTriangle2D(const Topology::Triangle2D& triangle){
    ...
    void Instance::DrawQuad(const Topology::Quad& quad){
        Matrix4x4 worldMat = Methods::Matrix::AffineMatrix(quad.scale, quad.rotate, quad.translate);
    ...
    void Instance::AddQuad3DPrimitive(
    ...
    void Instance::DrawQuad2D(const Topology::Quad2D& quad, const Color& masterColor){

        Matrix4x4 worldMat = Methods::Matrix::AffineMatrix(quad.scale.ToVec3(), { 0.0f,0.0f,quad.rotate }, quad.translate.ToVec3());
        instance_->pPolygonManager_->AddQuad(
    ...
    void Instance::DrawSprite(const Sprite& sprite, const std::optional<Color>& masterColor){
    ...
    void Instance::DrawModel(Model* model, const std::optional<Color>& masterColor){
    ...
    void Instance::DrawLine(const Vector3& v1, const Vector3& v2, const Color& color, BlendMode blendMode){
        instance_->pPolygonManager_->AddLine(
            Methods::Matrix::TransformToVec4(v1),
            Methods::Matrix::TransformToVec4(v2),
            Methods::Matrix::IdentityMat4(), color, true, true, blendMode, false
        );
    }

    void Instance::DrawLine2D(
        ...
        instance_->pPolygonManager_->AddLine(
            Methods::Matrix::TransformToVec4(v1),
            Methods::Matrix::TransformToVec4(v2),
            Methods::Matrix::IdentityMat4(), color, false, isApplyViewMat, blendMode, false, DrawLocation::Front, 100, isIgnoreDepth
        );
    }
    ...
    void Instance::DrawText2D(const TextBox2D& textBox){
        textBox.Draw();
    }

    void Instance::DrawText3D(const TextBox3D& textBox){
        textBox.Draw();
    }
    ...
    void Instance::DrawRing(const Topology::Ring& ring){
        ring;
    }
    ...
    ///////////////////////////////////////////////////////
    void Instance::DrawAABB(const Topology::AABB& aabb, const Color& color){
        Topology::OBB obb;
    ...
    void Instance::DrawAABB2D(const Topology::AABB2D& aabb, const Color& color, bool isIgnoreDepth){
        Vector2 vertex[4] = {
    ...
    void Instance::DrawOBB(const Topology::OBB& obb, const Color& color){
    ...
    // 2次元OBBの描画
    /////////////////////////////////////////////////////////////
    void Instance::DrawOBB2D(const Topology::OBB2D& obb, const Color& color, bool isIgnoreDepth){
        // 頂点を計算
    ...
    void Instance::DrawHexagonFrame(
    ...
    // 球の描画
    /////////////////////////////////////////////////////////////
    void Instance::DrawSphere(const Vector3& center, const Vector3& radius, int32_t subdivision, const Color& color){
    ...
    void Instance::DrawSphere(const Vector3& center, float radius, int32_t subdivision, const Color& color){
    ...
    void Instance::DrawCylinder(
        ...
        // 円柱の軸ベクトルと高さ
        Vector3 axis = Methods::Math::Normalize(end - start);

        // 基準となる法線ベクトル（円生成の基準）
        Vector3 up = (fabs(axis.y) < 0.99f) ? Vector3(0, 1, 0) : Vector3(1, 0, 0);
        Vector3 tangent = Methods::Math::Normalize(Methods::Math::Cross(up, axis));      // 円周の方向
        Vector3 bitangent = Methods::Math::Cross(axis, tangent);          // 円の垂直方向
    ...
    // カプセルの描画
    /////////////////////////////////////////////////////////////
    void Instance::DrawCapsule(const Vector3& start, const Vector3& end, float radius, int32_t subdivision, const Color& color){
        // 視点と終点に球を描画
    ...
    void Instance::DrawCapsule(const Vector3& start, const Vector3& end, const Vector3& radius, int32_t subdivision, const Color& color){
        // 視点と終点に球を描画
        DrawSphere(start, radius, subdivision, color);
        DrawSphere(end, radius, subdivision, color);
        // 円柱を描画
        DrawCylinder(start, end, Methods::Math::Length(radius), Methods::Math::Length(radius), subdivision, color);
    }
    ...
    // グリッドの描画
    /////////////////////////////////////////////////////////////
    void Instance::DrawGrid(float gridInterval, int32_t gridCount){
    ...
    void Instance::DrawBezier(const Vector3& p1, const Vector3& p2, const Vector3& p3, uint32_t subdivision, const Color& color){
        // ベジェ曲線の描画
        Vector3 previous = p1;
        for(uint32_t i = 1; i <= subdivision; i++){
            ...
            Vector3 current = Methods::Math::Bezier(p1, p2, p3, t);
    ...
    // 2D版
    void Instance::DrawBezier(const Vector2& p1, const Vector2& p2, const Vector2& p3, uint32_t subdivision, const Color& color){
        // ベジェ曲線の描画
        Vector2 previous = p1;
        for(uint32_t i = 1; i <= subdivision; i++){
            // 現在の位置を求める
            float t = float(i) / subdivision;
            Vector2 current = Methods::Math::Bezier(p1, p2, p3, t);
            // 線を描画
    ...
    // 複数点版
    void Instance::DrawBezier(const std::vector<Vector2>& points, uint32_t subdivision, const Color& color){
        ...
        for(uint32_t i = 0; i <= totalSubdivision; i++){
            ...
            // 現在の区間の点を求める
            Vector2 p = Methods::Math::Bezier(points, t);
            // 線を描画
    ...
    // スプライン曲線の描画
    /////////////////////////////////////////////////////////////
    void Instance::DrawSpline(const std::vector<Vector3>& points, uint32_t subdivision, const Color& color, bool isControlPointVisible){
        ...
        for(uint32_t i = 0; i <= totalSubdivision; i++){
            ...
            // 現在の区間の点を求める
            Vector3 p = Methods::Math::MultiCatmullRom(points, t);
    ...
    // スプライン曲線の描画（2D版）
    void Instance::DrawSpline(const std::vector<Vector2>& points, uint32_t subdivision, const Color& color, bool isControlPointVisible){
        ...
        for(uint32_t i = 0; i <= totalSubdivision; i++){
            ...
            // 現在の区間の点を求める
            Vector2 p = Methods::Math::MultiCatmullRom(points, t);
        ...
        // 制御点の描画
        if(!isControlPointVisible){ return; }
        Topology::Quad2D controlPointQuad = Methods::Shape::MakeEqualQuad2D(5.0f, { 1.0f,0.0f,1.0f,1.0f });
    ...
    // ライトのデバッグ用描画
    /////////////////////////////////////////////////////////////
    void Instance::DrawLight(const BaseLight* light){
        ...
        case LIGHT_TYPE::DIRECTIONAL_LIGHT:
        {
            ...
            Vector3 direction = Methods::Math::Normalize(directionalLight->direction_);
            float dot = Methods::Math::Dot(direction, -GetMainCamera()->GetNormal());
    ...
    void Instance::SetSkyBox(const std::string& textureName, const Color& color){
    ...
    void Instance::DrawSkyBox(bool isFollowCameraPos, const Vector3& position, float scale){
        SkyBox::scale_ = scale;
    ...
    Vector2 Instance::GetImageSize(const std::wstring& fileName){
    ...
    void Instance::ChangeResolutionRate(float resolutionRate){
    ...
    /*------------------ カメラにシェイクを設定する関数 ------------------*/
    void Instance::SetCameraShake(float time, float power, const Vector3& shakeLevel){
