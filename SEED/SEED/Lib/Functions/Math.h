...
namespace SEED{
    namespace Methods{
        namespace Math{
            ...
            // 2ベクトルの外積を求める関数 (3D)
            Vector3 Cross(const Vector3& vec1, const Vector3& vec2, bool kViewMode = Methods::Math::kWorld);
            ...
            Vector3 MultiCatmullRom(const std::vector<Vector3>& controlPoints, float t, bool isConnectEdge = false);
            Vector3 MultiCatmullRom(const std::vector<Vector3*>& controlPoints, float t, bool isConnectEdge = false);
            Vector2 MultiCatmullRom(const std::vector<Vector2>& controlPoints, float t, bool isConnectEdge = false);
            float MultiCatmullRom(const std::vector<float>& controlPoints, float t, bool isConnectEdge = false);
            // 制御点を等間隔に修正する関数
            ...
            Vector4 FloatColor(uint32_t color, bool isCorrectionToLiner = true);
            Vector4 FloatColor(uint32_t r, uint32_t g, uint32_t b, uint32_t a = 255, bool isCorrectionToLiner = true);
            // Vector4からカラーコードに変換する関数
            uint32_t IntColor(const Vector4& color);
            uint32_t IntColor(uint32_t r, uint32_t g, uint32_t b, uint32_t a = 255, bool isCorrectionToLiner = true);
