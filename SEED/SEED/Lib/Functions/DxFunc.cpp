    ...
    namespace Methods{
        namespace DxFunc{
            ...
            void Log(const std::wstring& message){
                OutputDebugStringA(String::ConvertString(message).c_str());
            }
            ...
            ComPtr<IDxcBlob> CompileShader(
                ...
                Log(String::ConvertString(std::format(L"Begin CompileShader, path: {}, profile : {}\n", filePath, profile)));
                ...
                assert(SUCCEEDED(hr));
                // 成功したログを出す
                Log(String::ConvertString(std::format(L"Compile Succeeded, path: {}, profile:{}\n", filePath, profile)));
                // もう使わないリソースを解放
                shaderSource.Reset();
                shaderResult.Reset();
                if(shaderError){
            ...
            ComPtr<ID3D12Resource> UploadTextureData(
                ...
                ComPtr<ID3D12Resource> intermediateResource = Methods::DxFunc::CreateBufferResource(device, intermediateSize);
