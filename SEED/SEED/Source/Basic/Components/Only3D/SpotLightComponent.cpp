...
namespace SEED{
    ...
    void SpotLightComponent::Update(){
        ...
        // transformの更新
        Matrix4x4 localRotMat = Methods::Matrix::RotateMatrix(localTransform_.rotate);
        ...
        // 親の回転を反映する場合
        if(isParentRotate_){
            positionVec *= Methods::Matrix::RotateMatrix(owner_.owner3D->GetWorldRotate());
            worldRotMat = localRotMat * Methods::Matrix::RotateMatrix(owner_.owner3D->GetWorldRotate());
        }
    ...
    void SpotLightComponent::EditGUI(){
        ...
        Matrix4x4 rotMat = Methods::Matrix::RotateMatrix(localTransform_.rotate);
