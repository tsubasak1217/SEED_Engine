...
namespace SEED{
        ...
        if(selectedObject_){
            ...
            if(selectedObject2D_){
                ...
                if(selectedObject2D_->GetParent()){
                    Matrix4x4 parentWorldMat = Methods::Matrix::ToMat4x4(selectedObject2D_->GetParent()->GetWorldMat());
    ...
    void Hierarchy::InOutOnGUI(){
        ...
        // Jsonファイルへの出力
        {
            ...
            if(ImGui::Button("シーンをJsonファイルへ出力")){
                // 最前面表示
                filename.clear();
                filename = Methods::File::OpenSaveFileDialog(outputFileDirectory, ".scene", "拡張子は不要です");
                ...
                if(!filename.empty()){
                    ...
                    // 出力
                    Methods::File::CreateJsonFile(filename, result);
                    // filenameをファイル名のみに戻す
    ...
    void Hierarchy::OutputPrefab(GameObject* gameObject){
        ...
        if(ImGui::BeginPopupModal("PrefabToJson", NULL, ImGuiWindowFlags_AlwaysAutoResize)){
            ...
            if(ImGui::Button("出力", ImVec2(120, 0))){
                filename.clear();
                filename = Methods::File::OpenSaveFileDialog(outputFileDirectory, ".prefab", "拡張子は不要です");

                // ダイアログ上で保存ボタンが押されたら保存処理を行う
                if(!filename.empty()){
                    std::list<GameObject*> outputObject;
                    outputObject.push_back(gameObject);
                    Methods::File::CreateJsonFile(filename, OutputToJson(outputObject, isSaveOnWorldOrigin));
                    // filenameをファイル名のみに戻す
                    filename = std::filesystem::path(filename).filename().stem().string();
                }
    ...
    LoadObjectData Hierarchy::LoadScene(const std::string& filePath, bool resetObjects){
        ...
        if(filePath.starts_with("Resources")){
            json = Methods::File::GetJson(filePath);
        } else{
            json = Methods::File::GetJson(sceneJsonDirectory_ + filePath);
        }
    ...
    GameObject* Hierarchy::LoadObject(const std::string& filePath){
        ...
        if(filePath.starts_with("Resources")){
            ...
            json = Methods::File::GetJson(prefabJsonDirectory_ + filePath);
        }
