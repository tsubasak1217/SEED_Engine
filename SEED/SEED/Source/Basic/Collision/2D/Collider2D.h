...
namespace SEED{
    ...
    public:
        ...
        virtual bool CheckCollision(const Topology::Quad2D& quad);
        virtual bool CheckCollision(const Topology::Circle& circle);
    ...
    public:// アクセッサ--------------------------------------------------------------
        ...
        // トランスフォーム取得
        Vector2 GetWoarldTranslate()const{ return Methods::Matrix::ExtractTranslation(worldMat_) + offset_; }
        Vector2 GetLocalTranslate()const{ return Methods::Matrix::ExtractTranslation(localMat_) + offset_; }
        void AddTranslate(const Vector2& addValue){ translate_ += addValue; }
        float GetWorldRotate()const{ return Methods::Matrix::ExtractRotation(worldMat_); }
        float GetLocalRotate()const{ return Methods::Matrix::ExtractRotation(localMat_); }
        Vector2 GetWorldScale()const{ return Methods::Matrix::ExtractScale(worldMat_); }
        Vector2 GetLocalScale()const{ return Methods::Matrix::ExtractScale(localMat_); }
        virtual bool IsMoved();
        const Vector2& GetCollidedPosition()const{ return collidedPosition_; }

        // 衝突判定八分木用
        const Topology::AABB2D& GetBox()const{ return coverAABB_; }
    ...
    protected:// 衝突に使用するパラメータ--------------------------------------------------
        Topology::AABB2D coverAABB_;// 頂点を包含するAABB
