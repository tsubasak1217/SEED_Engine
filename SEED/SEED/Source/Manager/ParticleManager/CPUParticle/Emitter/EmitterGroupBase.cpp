...
namespace SEED{
    ...
    void EmitterGroupBase::Edit(){
        ...
        for(int32_t i = 0; i < emitters.size(); i++){
            ...
            // 一意なタグの生成
            tag = "##" + Methods::String::PtrToStr(emitters[i].get());
    ...
    void EmitterGroupBase::OutputGUI(){
        ...
        // 型に応じて保存先を変更
        if(EmitterGroup3D* group3D = dynamic_cast<EmitterGroup3D*>(this)){
            outputDir = Methods::File::OpenSaveFileDialog("Resources/Jsons/Particle/3D", ".emtg3", outputFileName_);
        } else{
            outputDir = Methods::File::OpenSaveFileDialog("Resources/Jsons/Particle/2D", ".emtg2", outputFileName_);
        }

        if(!outputDir.empty()){
            Methods::File::CreateJsonFile(outputDir, GetJson());
        }
    #endif // _DEBUG
    }
